 ...
 * @license
 * SPDX-License-Identifier: Apache-2.0
*/const vt="AIzaSyALB2_8eh4kNA3hrpevwvDtgO6FnpkWU7s";let P=null;vt.trim()&&(P=new Dr({apiKey:vt}));const Ct={commitmentSigned:"roof-er.commitmentSigned"},W=document.getElementById("sidebar"),z=document.getElementById("main-content"),_={welcome:`
    <div class="content-card">
      ...
      <div class="leadership-grid">
        ...
        <div class="leader-card">
          ...
          <button class="bio-toggle-btn" onclick="toggleBio('oliver-bio')">My Bio</button>
        ...
        <div class="leader-card">
          ...
          <button class="bio-toggle-btn" onclick="toggleBio('reese-bio')">My Bio</button>
        ...
        <div class="leader-card">
          ...
          <button class="bio-toggle-btn" onclick="toggleBio('ford-bio')">My Bio</button>
...
`;_["sales-cycle-job-flow"]=_["sales-cycle"]||`
...
`;_["role-play"]=(_["role-play"]||"").replace('<div id="chat-container">',`<div class="rp-controls">
...
Be specific, actionable, and encouraging.`;try{let w=(await(await P.chats.create({model:"gemini-2.0-flash-exp",config:{temperature:.7,maxOutputTokens:500}})).sendMessage(v)).text.trim();return w.includes("```json")&&(w=((C=w.match(/```json\n([\s\S]*?)\n```/))==null?void 0:C[1])||w),JSON.parse(w)}catch{return{strengths:[`You scored ${y.score}/100`,`Matched ${y.matchedPoints.length} key points`],improvements:[`Try to include: ${y.missedPoints.slice(0,2).join(", ")}`,"Practice using clear, professional language"]}}}function u(h,m){const y=document.getElementById("score-circle"),v=document.getElementById("score-text"),C=document.getElementById("matched-points-list"),E=document.getElementById("missed-points-list"),T=document.getElementById("strengths-list"),w=document.getElementById("improvements-list");y&&(y.textContent=String(h.score),y.style.borderColor=h.score>=85?"#4caf50":h.score>=70?"#ff9800":"#f44336",y.style.color=h.score>=85?"#4caf50":h.score>=70?"#ff9800":"#f44336"),v&&(v.textContent=h.score>=70?`Great! You passed with ${h.score}/100`:`Score: ${h.score}/100 (Need 70 to pass)`),C&&(C.innerHTML=h.matchedPoints.length>0?h.matchedPoints.map(I=>`<li style="margin-bottom: 8px;"><span style="color: #4caf50; margin-right: 8px;">✓</span>${I}</li>`).join(""):"<li>No key points matched</li>"),E&&(E.innerHTML=h.missedPoints.length>0?h.missedPoints.map(I=>`<li style="margin-bottom: 8px;"><span style="color: #ff9800; margin-right: 8px;">✗</span>${I}</li>`).join(""):"<li>All key points covered!</li>"),m&&T&&w&&(T.innerHTML=m.strengths.map(I=>`<li style="margin-bottom: 8px;">${I}</li>`).join(""),w.innerHTML=m.improvements.map(I=>`<li style="margin-bottom: 8px;">${I}</li>`).join("")),t("feedback-area")}function c(){const h=o.currentScenarioIndex+1;h>=o.scenarios.length?f():(n(h),t("scenario-display"))}function d(){o.responses.length>0&&o.responses.pop(),o.scores.length>0&&o.scores.pop(),i(o.currentScenario),t("scenario-display")}function f(){t("session-summary");const h=document.getElementById("session-summary");if(!h)return;const m=o.scores.map(T=>T.score),y=m.length>0?Math.round(m.reduce((T,w)=>T+w,0)/m.length):0,v=m.length>0?Math.max(...m):0,C=m.filter(T=>T>=70).length;h.innerHTML=`
  ...
  `).join("")+'<button id="submitQuizButton">Submit Answers</button>',t.querySelectorAll(".quiz-options li").forEach(i=>{i.addEventListener("click",()=>{i.parentElement.querySelectorAll("li").forEach(a=>a.classList.remove("selected")),i.classList.add("selected")})});const n=document.getElementById("submitQuizButton");n==null||n.addEventListener("click",()=>{o.forEach((i,l)=>{const a=document.querySelector(`.quiz-item[data-question-index="${l}"] .quiz-options li.selected`),u=document.getElementById(`quiz-feedback-${l}`);a&&u&&(a.dataset.option===i.answer?(u.textContent="Correct!",u.className="quiz-feedback correct"):(u.textContent=`Incorrect. The correct answer is: ${i.answer}`,u.className="quiz-feedback incorrect"))}),n.disabled=!0})}function Ft(o){var t;if(z)switch(z.innerHTML=_[o]||"<div>Content not found.</div>",Y.speaking&&(Y.cancel(),$=null),o){case"quiz":(t=document.getElementById("generateQuizButton"))==null||t.addEventListener("click",Fr);break;case"sales-cycle":Tt();break;case"objection-handling":Et();break;case"role-play":Lr();break;case"welcome":Yr(),Br();break;case"general-knowledge":Hr();break;case"final-exam":$r();break;case"handling-initial-pitch-objections":Et();break;case"sales-cycle-job-flow":Tt();break;case"commitment":Gr();break}}function Vr(o){const t=o.target;if(t.tagName==="LI"&&t.dataset.module){const e=t.dataset.module;W==null||W.querySelectorAll("li").forEach(n=>n.classList.remove("active")),t.classList.add("active"),Ft(e)}}document.addEventListener("DOMContentLoaded",()=>{var o;W&&W.addEventListener("click",Vr),z==null||z.addEventListener("click",Nr),Ft("welcome"),(o=document.querySelector('#sidebar li[data-module="welcome"]'))==null||o.classList.add("active")});const Ur={oliver:{name:"Oliver Brown",title:"Owner & Founder",img:"/resources/images/oliver-theroofdocs.jpg",link:"https://www.theroofdocs.com/about/",summary:"Owner & Founder focused on integrity, quality, and simplicity with a transparent, customer‑first process."},reese:{name:"Reese Samala",title:"Director of Sales",img:"/resources/images/reese-theroofdocs.jpg",link:"https://www.theroofdocs.com/about/",summary:"Leads sales with a consultative, education‑forward approach that builds trust and results."},ford:{name:"Ford Barsi",title:"General Manager",img:"/resources/images/ford-theroofdocs.jpg",link:"https://www.theroofdocs.com/about/",summary:"Oversees operations and execution, aligning teams and process from inspection to completion."}};function Br(){var n;const o=document.getElementById("main-content");if(!o)return;let t=document.getElementById("bio-modal-overlay");t||(t=document.createElement("div"),t.id="bio-modal-overlay",t.className="modal-overlay",t.innerHTML=`
    ...
    `,(e=document.getElementById("quiz1Submit"))==null||e.addEventListener("click",()=>{var c,d,f;const n=(c=document.querySelector('input[name="qa1"]:checked'))==null?void 0:c.value,i=(d=document.querySelector('input[name="qa2"]:checked'))==null?void 0:d.value,l=(f=document.querySelector('input[name="qa3"]:checked'))==null?void 0:f.value,a=n==="a"&&i==="b"&&l==="c",u=document.getElementById("quiz1Result");if(u)if(a)u.textContent="✓ Perfect! You know the Roof-ER leadership team, core values, and founding year.",u.className="quiz-feedback correct";else{let p="✗ Not quite. ";n!=="a"&&(p+="Review the leadership team. "),i!=="b"&&(p+="Check our core values. "),l!=="c"&&(p+="Roof-ER was founded in 2019. "),u.textContent=p,u.className="quiz-feedback incorrect"}})})}
